...
docker run --link mariadb --link mongo --env-file .env --rm mklocke/liga-manager-api:$TAG sh -c "bin/generate-jwt-key.sh && bin/console.php orm:schema-tool:create && bin/console.php app:load-fixtures && bin/console.php orm:generate-proxies && phpunit"
